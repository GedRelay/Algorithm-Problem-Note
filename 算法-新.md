...
stringstream ss(line); // 或者ss.str(line);
...
[力扣 202. 快乐数](https://leetcode.cn/problems/happy-number/) 
...
int qselect(int a[], int l, int r, int k){
    ...
    if (k <= j) return qselect(a, l, j, k); // 如果k在分界点左边，则在左边找第k大的数
    else return qselect(a, j + 1, r, k); // 否则在右边找第k大的数
}
```
...
LL mergesort(int a[], int l, int r){
    if(l == r) return 0;
    int mid = l + r >> 1;
    LL res = mergesort(a, l, mid) + mergesort(a, mid + 1, r); // 递归求解左右两部分的逆序对数量
...
[力扣 121. 买卖股票的最佳时机](https://leetcode.cn/problems/best-time-to-buy-and-sell-stock/) （线性 DP / 贪心  + 前缀最值 / 双指针）
...
## 双指针
核心思想是将枚举序列两个端点的暴力算法通过某些性质从 ${O\left( n^{2}  \right)  }$ 优化到 ${O\left( n \right)  }$，最外面一层循环枚举一个端点 ${i }$，再通过某些性质使得第二个指针 ${j }$ 只在一个方向上移动而不会走回头路
- 时间复杂度：${O\left( n \right)  }$ 
```cpp
// 暴力算法枚举两个端点
for(int i = 0; i < n; i++){
...
[力扣 82. 删除排序链表中的重复元素 II](https://leetcode.cn/problems/remove-duplicates-from-sorted-list-ii/) 
...
[力扣 457. 环形数组是否存在循环](https://leetcode.cn/problems/circular-array-loop/) 
...
- 时间复杂度：插入 ${O\left( n \right)  }$，查找 ${O\left( n \right)  }$，${n }$ 为字符串总长度
- 空间复杂度：${O\left( n \right)  }$，${n }$ 为字符串总长度
```cpp
// 该模板只支持小写英文字母
// N为字符串最长长度
struct Trie{
    int son[N][26] = {0}; // son[i][j]表示第i个节点的第j个儿子的编号
    int cnt[N] = {0}; // cnt[i]表示以第i个节点为结尾的单词的数量
    int id = 0; // 记录节点编号，只对出现过的节点编号
    void insert(string word){ // 插入单词
        int p = 0;
        for(char& c : word){
            int u = c - 'a';
            if(!son[p][u]) son[p][u] = ++id; // 对第一次出现的节点进行编号
            p = son[p][u];
        }
        cnt[p]++;
    }
    int find(string word){// 查找单词出现的次数
        int p = 0;
        for(char& c : word){
            int u = c - 'a';
            if(!son[p][u]) return 0;
...
用于稀疏图
- 时间复杂度：${O\left( n+m\log n \right)  }$，${n }$ 为点数，${m }$ 为边数
```cpp
// 堆优化dijkstra
vector <PII> e[N]; // 邻接表存储 {距离，点编号}
int dis[N]; // 起点到每个点的距离
bool vis[N]; // 表示是否在集合内
...
[力扣 1976. 到达目的地的方案数](https://leetcode.cn/problems/number-of-ways-to-arrive-at-destination/) 


### Bellman-Ford

求单源最短路径，有边数限制的单源最短路径
1. 循环 ${k }$ 次（表示每条路径的边数不超过 ${k }$）
	遍历所有边 ${a\stackrel{w}{\longrightarrow}b }$ 
		${dis\left[ b \right] =\min\{ dis\left[ b \right] ,dis\left[ a \right] +w \}  }$ 
...
for(int i = 0; i < k; i++){
    memcpy(backup, dis, sizeof dis); // 备份，只使用上一次的值来更新
    for(int j = 0; j < m; j++){
...
### floyd
求多源最短路径（要求图中不能存在负权回路）
- 时间复杂度：${O\left( n^{3}  \right)  }$ ，${n }$ 为点数
```cpp
for (int k = 1; k <= n; k++) 
	for (int i = 1; i <= n; i++) 
		for (int j = 1; j <= n; j++) 
...
- 状态表示：$dp[j]$ 表示背包容量为 $j$ 时背包最多能装下的最大价值
- 初始状态：${dp\left[ 0 \right] =0 }$ 
- 动态转移：$dp[j]=max(dp[j],dp[j-w[i]\times k]+v[i]\times k)$ 
- 时间复杂度：$O(nTs)$
```cpp
for (int i = 1; i <= n; i++) // 依次枚举每个物品
    for (int j = T; j >= w[i]; j--) // 容量从右往左更新
        for (int k = 0; k <= s[i] && w[i] * k <= j; k++) // 第i个物品选k件
            dp[j] = max(dp[j], dp[j - w[i] * k] + v[i] * k);
```

二进制优化：
主要思路是将物品以二进制的方式进行拆分，比如某样物品的个数为 ${15 }$ 个，则可用拆成以下 ${4 }$ 组，每组有 ${1,2,4,8 }$ 个物品。拆分后转化为 0-1 背包问题
- 时间复杂度：${O\left( nT\log s \right)  }$ 
```cpp
// 拆分
vector<PII> goods; // weight, value
for(int i = 0; i < n; i++){
    int w, v, s;
    cin >> w >> v >> s;
    // 对s进行二进制拆分，每j个拆分出来一个新的物品
    for(int j = 1; j <= s; j *= 2){
...
for(int i = 0; i < n; i++){
	...
	if(l >= f.size() || f[l] < a[i]) f.push_back(a[i]);
	else f[l] = a[i];
}
```
...
#### dilworth 定理
1. 用最长不上升序列覆盖整个序列，所需要的最少个数，等于最长上升序列长度
2. 用最长上升序列覆盖整个序列，所需要的最少个数，等于最长不上升序列长度
如：1, 7, 8, 2, 3, 4
...
[洛谷 P1036. 选数](https://www.luogu.com.cn/problem/P1036) （DFS+判断质数）
[洛谷 P2036. PERKET](https://www.luogu.com.cn/problem/P2036) （状态压缩+组合数）

博弈
[洛谷 P4702. 取石子](https://www.luogu.com.cn/problem/P4702) 

dp
[洛谷 P1244. 青蛙过河]( https://www.luogu.com.cn/problem/P1244 ) 
[洛谷 P1387. 最大正方形](https://www.luogu.com.cn/problem/P1387) 
[洛谷 P2701. 巨大的牛棚]( https://www.luogu.com.cn/problem/P2701 ) 
[洛谷 P1990. 覆盖墙壁](https://www.luogu.com.cn/problem/P1990) 
[洛谷 P1352. 没有上司的舞会](https://www.luogu.com.cn/problem/P1352) 
[洛谷 P1003.铺地毯](https://www.luogu.com.cn/problem/P1003) （二维差分？小技巧？）

递归
[洛谷 P1030. 求先序排列]( https://www.luogu.com.cn/problem/P1030 ) 
[洛谷 P1305. 新二叉树](https://www.luogu.com.cn/problem/P1305) 
[洛谷 P1827. 美国血统]( https://www.luogu.com.cn/problem/P1827 ) 
[洛谷 P1087. FBI 树]( https://www.luogu.com.cn/problem/P1087 ) 


[洛谷 P5318. 查找文献](https://www.luogu.com.cn/problem/P5318)（图+dfs+bfs）
...
?
[洛谷 P2234. 营业额统计]( https://www.luogu.com.cn/problem/P2234 ) 
[洛谷 P1137. 旅行计划](https://www.luogu.com.cn/problem/P1137) 
[洛谷 P2879. Tallest Cow S]( https://www.luogu.com.cn/problem/P2879 ) 
...
数学，快速幂
[洛谷 P3197. 越狱]( https://www.luogu.com.cn/problem/P3197 ) 

背包方案数
[洛谷 P1164. 小A点菜](https://www.luogu.com.cn/problem/P1164) 
[acwing 1371. 货币系统](https://www.acwing.com/problem/content/description/1373/) 
...
不会做
[洛谷 P1249. 最大乘积](https://www.luogu.com.cn/problem/P1249) （贪心+高精度）
[洛谷 P1045. 麦森数]( https://www.luogu.com.cn/problem/P1045 ) （高精度）
...
## 状态压缩八皇后
