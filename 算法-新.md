...
[力扣 719. 找出第 K 小的数对距离](https://leetcode.cn/problems/find-k-th-smallest-pair-distance/) （二分答案 + 滑动窗口）
[力扣 2302. 统计得分小于 K 的子数组数目](https://leetcode.cn/problems/count-subarrays-with-score-less-than-k/) 
...
```
[力扣 1423. 可获得的最大点数](https://leetcode.cn/problems/maximum-points-you-can-obtain-from-cards/) （双指针 / 前缀和）
[力扣 2269. 找到一个数字的 K 美丽值](https://leetcode.cn/problems/find-the-k-beauty-of-a-number/) 
...
[力扣 LCR 014. 字符串的排列](https://leetcode.cn/problems/MPnaiL/) 
...
[力扣 881. 救生艇](https://leetcode.cn/problems/boats-to-save-people/) （贪心 + 双指针）
...
思路：一个数入栈之前，将以后再也不可能会成为答案的数字全部从栈顶弹出。栈顶就是左边第一个大于/小于待入栈的数。 (要求右边反向遍历即可)
- 时间复杂度：${O\left( n \right)  }$ 
```cpp
// 求左边第一个比a[i]小的数的下标
stack<int> s;
...
[力扣 1019. 链表中的下一个更大节点](https://leetcode.cn/problems/next-greater-node-in-linked-list/) 
...
## 单调队列
### 求滑动窗口中的最值
思路：使用双端队列，一个数从队尾入队之前，将以后再也不可能会成为答案的数字全部从队尾出队。然后判断队头是否过期（超出窗口），如果过期则将队头从队头出队，最后队头就是滑动窗口的最小值/最大值。
...
[力扣 LCR 183. 望远镜中最高的海拔](https://leetcode.cn/problems/hua-dong-chuang-kou-de-zui-da-zhi-lcof/) 
...
[洛谷 P4391【BalticOI 2009】Radio Transmission 无线传输](https://www.luogu.com.cn/problem/P4391) （next 数组找循环节）
[洛谷 P4824【USACO15FEB】Censoring S](https://www.luogu.com.cn/problem/P4824) （栈 + KMP）
[力扣 1367. 二叉树中的链表](https://leetcode.cn/problems/linked-list-in-binary-tree/) （递归 + KMP）
...
[力扣 952. 按公因数计算最大组件大小](https://leetcode.cn/problems/largest-component-size-by-common-factor/) （质因数分解 + 并查集）
...
思路：把字符串看作一个 ${P }$ 进制数，对其取模得到哈希值（经验值： ${P }$ 取 ${131,13331,499 }$，模数取 ${2^{64}  }$，使用 `unsigned long long` 类型时数据溢出相当于对 ${2^{64}  }$ 取模，因此不需要额外取模了）
...
### 字符串前缀哈希
任意区间字符串的哈希值可以由前缀字符串的哈希值计算得到，只需要预处理出前缀哈希值，之后获取某个子串的哈希值可以直接计算得到
...
const int P = 131;//经验值
ull h[N], p[N];// h是前缀哈希表，p[i]表示p的i次方
...
[力扣 1863. 找出所有子集的异或总和再求和](https://leetcode.cn/problems/sum-of-all-subset-xor-totals/) 
...
题目：
[力扣 994. 腐烂的橘子](https://leetcode.cn/problems/rotting-oranges/) 
...
[力扣 1439. 有序矩阵中的第 k 个最小数组和](https://leetcode.cn/problems/find-the-kth-smallest-sum-of-a-matrix-with-sorted-rows/) 
[力扣 2386. 找出数组的第 K 大和](https://leetcode.cn/problems/find-the-k-sum-of-an-array/) 


## 马拉车 manacher
在字符串中，找出所有回文串（计算扩展串中每个字符的回文半径）
原回文长度 = 回文半径 - 1
...
[力扣 LCR 020. 回文子串](https://leetcode.cn/problems/a7VOhD/) 
[力扣 5. 最长回文子串](https://leetcode.cn/problems/longest-palindromic-substring/) 
[力扣 647. 回文子串](https://leetcode.cn/problems/palindromic-substrings/) 

## ST 表
ST 表利用倍增思想和动态规划，是用于解决**可重复贡献问题**的数据结构，如最大值、最小值、最大公约数、最小公倍数、按位或、按位与等
可重复贡献问题：${f\left( A\cup B \right) =f\left( f\left( A \right) ,f\left( B \right)  \right)  }$，其中 ${A\cap B\neq \emptyset }$ 
...
树状数组有两个操作，这两个操作都由 ${lowbit }$ 作为基础。
1. 在某个位置 ${x }$ 加上一个数 ${v }$ 
2. 求区间 ${\left[ 1,x \right]  }$ 的前缀和

![image.png](https://ged-pic-bed.oss-cn-guangzhou.aliyuncs.com/img/202407171445025.png)


树状数组可以实现以下三种修改和查询操作，每个操作时间复杂度都是 ${O\left( \log n \right)  }$ 
1. 单点修改，区间查询
2. 区间修改，单点查询
3. 区间修改，区间查询（建议直接上线段树）
...
- 空间复杂度：${O\left( n \right)  }$ 
- ==注意：==数组下标一定要从 1 开始，不然 `lowbit` 出问题
```cpp
struct BIT{
    int t[N];  // 下标一定要从1开始
    // 核心操作
...
struct BIT{
    int t[N];  // 下标一定要从1开始
    int lowbit(int x){...}  // 核心操作
    void add(int x, int v){...}  // 单点修改：第x个数加上v
    int presum(int x){...}  // 前缀和：求区间[1, x]的前缀和
...
### 二维单点修改，区间查询
线段树解决二维问题需要用到树套树，较麻烦，所以若二维问题可以用树状数组解决（满足结合律和可差分）则建议用树状数组

- 时间复杂度：
	- 单点修改 ${O\left( \log n \log m\right)  }$ 
	- 区间查询 ${O\left( \log n\log m \right)  }$
...
题目：
[LibreOJ 133. 二维树状数组 1：单点修改，区间查询](https://loj.ac/p/133) 


### 二维区间修改，区间查询
- 时间复杂度：
	- 区间修改 ${O\left( \log n \log m\right)  }$ 
	- 区间查询 ${O\left( \log n\log m \right)  }$ 
- 空间复杂度：${O\left( nm \right)  }$ 
```cpp
struct BIT{
    int t1[N][M];  // 维护d[i][j]
    int t2[N][M];  // 维护d[i][j] * i
    int t3[N][M];  // 维护d[i][j] * j
    int t4[N][M];  // 维护d[i][j] * i * j
    // 核心操作
    int lowbit(int x){
        return x & -x;
    }
    // 单点修改：差分数组上的(x,y)位置加上v
    void add(int x, int y, int v){
        int v1 = v;
        int v2 = v * x;
        int v3 = v * y;
...
题目：
[洛谷 P4514 上帝造题的七分钟](https://www.luogu.com.cn/problem/P4514) 
...
[力扣 935. 骑士拨号器](https://leetcode.cn/problems/knight-dialer/) 
...
[力扣 2127. 参加会议的最多员工数](https://leetcode.cn/problems/maximum-employees-to-be-invited-to-a-meeting/) （拓扑排序 + 基环树）
...
- 预估函数要求：当前点到终点的预估距离 ${\leq  }$ 当前点到终点的真实最短距离（不能高估）
- 预估距离越接近真实最短距离，效率越快
预估函数通常有：
- 曼哈顿距离
```cpp
...
## 基环树
n 个点 n 条边的联通无向图，称为基环树
![image.png](https://ged-pic-bed.oss-cn-guangzhou.aliyuncs.com/img/202504221300061.png)
...
[洛谷 179. 最大数](https://leetcode.cn/problems/largest-number/) 
...
- **同加性**：若 ${x\equiv y\left( \mod{M}  \right)  }$，则 ${x\pm z\equiv y\pm z\left( \mod{M}  \right)  }$ 
...
[力扣 2614. 对角线上的质数](https://leetcode.cn/problems/prime-in-diagonal/) 
...
- 时间复杂度：${O\left( \sqrt{ n }  \right)  }$ ，如果已经打了一个质数表的话，时间复杂度将降低为 ${O\left( \sqrt{ \frac{n}{\ln n}  }  \right)  }$ 
```cpp
...
求出 ${1\sim n }$ 中所有的质数
#### 埃氏筛
-  时间复杂度：${O\left( n\log \left( \log n \right)  \right)  }$
```cpp
...
vector<int> get_primes(int n){
    ...
    for(int i = 2; i <= n; i++){
        if(!st[i]){
            primes.push_back(i);
            for(long long j = 1ll * i * i; j <= n; j += i){  // 使用long long防止爆int
                st[j] = true;
            }
        }
    }
...
[acwing 197. 阶乘分解](https://www.acwing.com/problem/content/description/199/) 
...
如果要计算除法的模：
$$
\frac{a}{b} \%M=\left( a\%M\times b^{-1} \left( \text{mod} \ M \right) \right)  \%M=\left( a\%M\times b^{M-2} \%M \right) \%M
$$
其中 ${b^{-1} \left( \text{mod}\ M  \right)  }$ 叫做 ${b }$ 在模 ${M }$ 意义下的乘法逆元，${b^{-1} \left( \text{mod} M \right) =b^{M-2} \%M }$ 
上式成立需要满足三个条件：
1. ${b|a }$，即 `a%b==0` （第一个等号成立的条件：除法改写为乘法）
2. ${b }$ 与 ${M }$ 互质，即 `gcd(b,M)==1`，若 ${M }$ 是质数则只需满足 `b%M!=0`（乘法逆元存在的条件）
3. ${M }$ 是质数（第二个等式成立的条件：用来计算乘法逆元）
...
### 预处理乘法逆元
递推预处理 ${1\sim n }$ 所有整数在模 ${p }$ 意义下的乘法逆元
$$
x^{-1}\left( \text{mod} \ p \right) =-\left\lfloor \frac{p}{x}  \right\rfloor \times \left( p\%x \right) ^{-1} \left( \text{mod\ p}  \right) 
$$
- 时间复杂度：${O\left( n \right)  }$ 
```cpp
...
[洛谷 P3811【模板】模意义下的乘法逆元](https://www.luogu.com.cn/problem/P3811) 

### 预处理阶乘乘法逆元
递推预处理 ${0!,1!,2!,\cdots ,n! }$ 在模 ${p }$ 意义下的乘法逆元
- 时间复杂度：${O\left( n+\log p \right)  }$ 
```cpp
int jc[N], jc_inv[N];
...
### 利用递推预处理组合数
当数据范围 ${n,m }$ 在 ${2000 }$ 数量级时，可以使用递推式将所有组合数预处理出来
$$
...
### 利用公式计算组合数
预处理阶乘逆元，再使用公式计算组合数：
当数据范围 ${n,m }$ 在 ${1e5 }$ 数量级时，可以预处理出阶乘以及阶乘逆元，利用组合计算公式进行计算
$$
...
不预处理直接计算组合数
- 时间复杂度：计算组合数 ${O\left( m\log p \right)  }$ 
```cpp
int C(int n, int m, int p){
    long long res = 1;
...
### lucas 卢卡斯定理求组合数
当数据范围 ${n,m }$ 在 ${1e18 }$ 数量级，但是模数 ${p }$ 在 ${1e5 }$ 数量级时，可以使用 lucas 定理求解组合数
lucas 定理：
$$
\Large{ C_{n} ^{m} \%p=C_{n\%p} ^{m\%p} \times C_{\left\lfloor \frac{n}{p}  \right\rfloor } ^{\left\lfloor \frac{m}{p}  \right\rfloor } \%p }
$$
- 时间复杂度：${O\left( g\left( p \right) \log _{p} n  \right)  }$，${g\left( p \right)  }$ 为单次计算组合数的复杂度
```cpp
...
[洛谷 P3807【模板】卢卡斯定理Lucas 定理](https://www.luogu.com.cn/problem/P3807) 

### 高精度求组合数
方法 1：实现高精度乘法和高精度除法，然后根据组合数的计算公式计算
方法 2：统计出 ${C_{n} ^{m}  }$ 中每个质因子以及个数，然后使用高精度乘法计算。只需统计出阶乘中的某个质数的数量即可
...
$$
SG\left( X \right) =\text{mex} \left( SG\left( a \right) ,SG\left( b \right) ,\cdots ,SG\left( z \right)  \right) ,\quad a,b,c,\cdots ,z为X的后继结点
$$
$$
\text{mex} \left( 数字集合 \right) =数字集合中没有出现过的最小自然数
$$

若 ${SG\left( X \right) \neq 0 }$，那么 ${X }$ 为必胜态，若 ${SG\left( X \right) =0 }$，则 ${X }$ 为必败态

### SG 定理（Bouton 定理）
如果一个 ICG 游戏由若干个独立的 ICG 游戏构成，每位玩家每个回合在这若干个子游戏中选择一个，对它进行一次操作，当所有子游戏都无法进行操作时，当前玩家输掉游戏。则有
$$
SG\left( 总 \right) =SG\left( 分1 \right) \oplus SG\left( 分2 \right) \oplus\cdots \oplus SG\left( 分n \right) 
$$
其中 ${\oplus }$ 为异或运算
...
## Bash 巴什博奕
一共有 ${n }$ 颗石子，两名玩家轮流行动，每次可以取走 ${1\sim m }$ 颗石子，最后无法进行操作的人视为失败。问先手是否必胜？
...
## Nim 尼姆博弈
给定 ${n }$ 堆物品，第 ${i }$ 堆物品有 ${a\left[ i \right]  }$ 个。两名玩家轮流行动，每次可以任选一堆, 取走任意多个物品，最后无法进行操作的人视为失败。问先手是否必胜？
...
### 反 Nim 博弈
给定 ${n }$ 堆物品，第 ${i }$ 堆物品有 ${a\left[ i \right]  }$ 个。两名玩家轮流行动，每次可以任选一堆, 取走任意多个物品，最后无法进行操作的人视为胜利。问先手是否必胜？

结论：
当每堆石子都只有 $1$ 个的时候分奇偶讨论，奇数必败，偶数必胜，否则求异或和，若异或和 $\neq 0$ 则先手必胜

题目：
[洛谷 P4279【SHOI2008】小约翰的游戏](https://www.luogu.com.cn/problem/P4279) 

### 台阶 Nim 
给定 ${n }$ 堆物品，第 ${i \left( 1,2,3\cdots  \right) }$ 堆物品有 ${a\left[ i \right]  }$ 个。两名玩家轮流行动，每次可以任选一堆, 取走任意多个物品放到前一堆中，第 ${0 }$ 堆的物品不能动，最后无法进行操作的人视为失败。问先手是否必胜？

结论：
所有奇数位置的异或和 ${\neq 0 }$ ，先手必胜

题目：
[acwing 892. 台阶-Nim 游戏](https://www.acwing.com/problem/content/894/) 

## 斐波那契博弈
一共有 ${n }$ 颗石子，两名玩家轮流行动，第一次可以取走任意多个石子，接下来每次至少要取走一个石子，最多取走上一次取的数量的 2 倍。取走最后一块石子的玩家获胜。显然先手必胜。问先手第一次至少要取走几颗石子才能保证必胜？
...
题目：
[洛谷 P6487【COCI 2010/2011 #4 】HRPA](https://www.luogu.com.cn/problem/P6487) 


## 威佐夫博弈
有两堆物品，物品数量分别为 ${a,b }$，两名玩家轮流行动，行动从以下两个来选择：
1. 选择一堆，可以取走这堆任意个物品
2. 从两堆中取走同样数量的物品
最后无法进行操作的人视为失败。问先手是否必胜？

结论：
${\left\lfloor \frac{\sqrt{ 5 } +1}{2} \times \left( b-a \right)  \right\rfloor \neq a }$ ，先手必胜（假定 $b>=a$）

[洛谷 P2252【SHOI2002】取石子游戏|【模板】威佐夫博弈](https://www.luogu.com.cn/problem/P2252) 

## 其他博弈
共有 n 个石子，两人每次都只能取 ${p^{k}  }$ 个（ ${p }$ 为质数，${k }$ 为自然数 (${0,1,2,\cdots  }$)，且 ${p^{k} \leq  }$ 当前剩余石子数），谁取走最后一个石子，谁就赢了。问先手是否必胜？

结论：
${n\%6\neq 0 }$ 先手必胜

题目：
[洛谷 P4018 Roy&October 之取石子](https://www.luogu.com.cn/problem/P4018) 
...
[力扣 1721. 交换链表中的节点](https://leetcode.cn/problems/swapping-nodes-in-a-linked-list/) 
...
## dfn 序
将树通过深度优先搜索（先序遍历），每个结点被访问的顺序为每个结点的 dfn 序。如果再结合以某个结点为根的子树的结点个数，有以下性质：
- 以某个结点为根的子树的 dfn 序是连续的，范围为 ${dfn\left[ root \right] \sim  dfn\left[ root \right] + size\left[ root \right]  -1  }$ 
可以根据这一性质判断一个结点是否是另一个结点的祖先
可以根据这一性质将子树的每个结点信息转换为连续的区间问题

题目：
[力扣 2458. 移除子树后的二叉树高度](https://leetcode.cn/problems/height-of-binary-tree-after-subtree-removal-queries/) 
